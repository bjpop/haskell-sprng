...
static int spawn_rng(LFG *, int, LFG ***);
			 ...
			 unsigned *nstart_local, unsigned initseed_local)
{
  ...
  order = (int *) malloc(ngen_local*sizeof(int));
  q = (LFG **) malloc(ngen_local*sizeof(LFG *));
...
/* for the sake of the Haskell binding */
void free_rng(LFG *lfg)
{
   free(lfg);
}

//int LFG::init_rng(int g, int tg, int s, int pa)
int init_rng(LFG *gen, int g, int tg, int s, int pa)
{
...
/*      check values of gennum and total_gen                             */
  ...
  if (pa < 0 || pa >= NPARAMS) 
  {
    ...
    fprintf(stderr, "WARNING - init_rng Parameter not valid. Using Default param");
    pa = 0;
  }
...
/*            the 1st time                                               */
  ...
  else 
  {
	...
	fprintf (stderr, "WARNING - init_rng changing global L value! Independence of streams is not guaranteed");
      if (doexit&2) 
	//errprint("WARNING","init_rng","changing global seed value! Independence of streams is not guaranteed");
	fprintf(stderr, "WARNING - init_rng changing global seed value! Independence of streams is not guaranteed");
    }
  }
  ...
  gen->rng_type = 0;
  gen->gentype = p[0]->gentype;
  gen->si = p[0]->si;
  gen->r0 = p[0]->r0;
  gen->r1 = p[0]->r1;
  gen->stream_number = p[0]->stream_number;
  gen->hptr = p[0]->hptr;
  gen->seed = p[0]->seed;
  gen->init_seed = p[0]->init_seed;
  gen->lval = p[0]->lval;
  gen->kval = p[0]->kval;
  gen->param = p[0]->param;
...
// for the sake of the Haskell FFI
LFG **spawn_rng_wrapper(LFG *gen, int num)
{
   LFG **new_rngs;
   spawn_rng(gen, num, &new_rngs);
   return new_rngs;
}

void free_spawn_buffer(LFG **buffer)
{
   free(buffer);
}

//int LFG::spawn_rng(int nspawned, Sprng ***newgens)
int spawn_rng(LFG *gen, int nspawned, LFG ***newgens)
{
  LFG **q = NULL;
  //int i;
  unsigned *p;
  ...
  p = gen->si;
  q = initialize(gen->rng_type,nspawned,gen->param,gen->seed,p,gen->init_seed);
  ...
  si_double(p,p,gen->lval);

  NGENS += nspawned; //XXX this does not look thread safe
